'''adapted from https://github.com/Sunarker/GAT-for-PPI/blob/master/utils/process_inductive.py'''
import json
import os
import pdb
import pickle
...
def load_ppi(data_dir, ds_name='ppi'):
    print('Loading G...')
    g_name = f'{ds_name}-G.json'
    with open(os.path.join(data_dir, g_name)) as jsonfile:
    ...
    id_map_name = f'{ds_name}-id_map.json'
    with open(os.path.join(data_dir, id_map_name)) as jsonfile:
    ...
    feats_name = f'{ds_name}-feats.npy'
    features_ = np.load(os.path.join(data_dir, feats_name))
    ...
    cls_map_name = f'{ds_name}-class_map.json'
    print('Loading class_map...')
    with open(os.path.join(data_dir, cls_map_name)) as jsonfile:
...
if __name__ == '__main__':
    data = load_ppi('../data/ppi')

    out_path = '../data/processed/ppi/'
    ...
    split_data = ([], [], [])
    for i in range(0, n):
        split_data[i % 3].append(data[i])

    for i, name in enumerate(['train', 'val', 'test']):
        with open(out_path + f'ppi_{name}.pkl', 'wb') as f:
            pickle.dump(split_data[i], f)
